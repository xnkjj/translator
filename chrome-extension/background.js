...
async function handleTranslation(textToTranslate, tabId) {
  const {apiKey, prompt} = await new Promise((resolve) => {
  ...
  const requestUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:streamGenerateContent?alt=sse&key=${apiKey}`;
  const requestBody = {
    contents: [
      {
        role: "user",
